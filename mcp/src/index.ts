...
server.tool(
  ...
  async ({ name, unitPrice, monthlyNewCustomers, monthlyChurnRate }) => {
    const data = loadData() as Record<string, unknown>;
    const rm = (data.revenueModel as Record<string, unknown>) ?? { type: "mrr", plans: [] };
    const plans = (rm.plans as unknown[]) ?? [];
    plans.push({
...
server.tool(
  ...
  async ({ name, monthlyAmount }) => {
    const data = loadData() as Record<string, unknown>;
    const costs = (data.fixedCosts as unknown[]) ?? [];
...
server.tool(
  ...
  async ({ initialCash, simulationMonths }) => {
    const data = loadData();
    if (initialCash != null) data.initialCash = initialCash;
    if (simulationMonths != null) data.simulationMonths = simulationMonths;
    saveData(data);
    const parts: string[] = [];
    if (initialCash != null) parts.push(`自己資本: ${(initialCash / 10000).toFixed(0)}万円`);
    if (simulationMonths != null) parts.push(`期間: ${simulationMonths}ヶ月`);
    return {
...
server.tool(
  "import_json",
  "MikomiのWeb UIからエクスポートしたJSONデータをインポートする",
  {
...
server.tool(
  "export_json",
  "Web UIにインポートできるJSON形式でエクスポートする",
  {},
  async () => {
    const data = loadData();
